...
//#region node_modules/.nitro/vite/services/ssr/assets/ofc360-store-BR2yEBkC.js
...
var BASE_URL = `${"https://api.ofc360.com".replace(/\/$/, "")}/api/v1`;
...
var fetchDepartments = createAsyncThunk("departments/fetchDepartments", async (_, { rejectWithValue }) => {
	try {
		const response = await apiInstance.get("/departments");
		const items = response.data?.data?.items ?? response.data?.data ?? response.data ?? [];
		return syncWithEmployees((Array.isArray(items) ? items : []).filter((d) => d && typeof d === "object" && d.name && d.name.trim() !== "" && d.code && d.code.trim() !== "" && d.status));
	} catch (err) {
		return rejectWithValue(err?.response?.data?.message || err?.message || "Failed to fetch departments");
	}
});
var createDepartment = createAsyncThunk("departments/createDepartment", async (department, { rejectWithValue }) => {
	if (!department.name?.trim() || !department.code?.trim() || !department.status) return rejectWithValue("Department requires name, code, and status.");
...
var importDepartments = createAsyncThunk("departments/importDepartments", async (departments, { rejectWithValue }) => {
	const validDepts = departments.filter((d) => d && d.name?.trim() && d.code?.trim() && d.status);
	if (validDepts.length === 0) return rejectWithValue("No valid departments to import.");
	await tryApi(() => apiInstance.post("/departments/import", { departments: validDepts }), void 0);
	return validDepts;
});
...
//#endregion
export { fetchEmployees as $, useGetHistoryQuery as $t, avatarHue as A, resetEmployeePassword as At, createEmployee as B, updateGoal as Bt, addReward as C, ofc360 as Ct, archiveJob as D, rememberStore as Dt, applySorting as E, promoteDepartmentEmployee as Et, bulkSetDepartmentStatus as F, store as Ft, deleteDepartment as G, upsertInterview as Gt, createManager as H, updateReview as Ht, bulkSetReviewStatus as I, transferDepartmentEmployees as It, deleteJob as J, useClearHistoryMutation as Jt, deleteEmployee as K, upsertJob as Kt, clearSelectedManager as L, uid as Lt, bulkAssignDepartmentManager as M, resolveDepartmentValue as Mt, bulkDeleteDepartments as N, setManagerForm as Nt, assignGoal as O, removeEmployeeFromDepartment as Ot, bulkDeleteReviews as P, setTokens as Pt, fetchDepartments as Q, useGetDepartmentReportsQuery as Qt, completeGoal as R, updateDepartment as Rt, addNote as S, moveStage as St, applyFilters as T, optimisticUpsertInterview as Tt, createReview as U, updateTrainingStatus as Ut, createGoal as V, updateManager as Vt, deactivateEmployee as W, upsertCandidate as Wt, deleteReview as X, useGetAttendanceReportsQuery as Xt, deleteManager as Y, useExportReportsMutation as Yt, duplicateJob as Z, useGetDashboardReportsQuery as Zt, SHIFT_OPTIONS as _, isAccessTokenExpired as _t, DEPARTMENTS as a, validatePhone as an, fetchRecruitmentData as at, addEmployeeToDepartment as b, loadOfc360Store as bt, EMPLOYMENT_TYPE_OPTIONS as c, getDepartmentLabel as ct, MANAGER_FORM_WORK_LOCATION_OPTIONS as d, getTokens as dt, useMounted as en, fetchJobById as et, OFFICES as f, getVisiblePages as ft, SHIFTS as g, initManagerForm as gt, REVIEW_STATUS_OPTIONS as h, importReviews as ht, DEFAULT_PERMISSIONS as i, validateManagerForm as in, fetchPerformance as it, buildCSV as j, resetManagerForm as jt, assignTraining as k, resendEmployeeInvite as kt, GENDER_OPTIONS as l, getErrorMessage as lt, RATING_BADGES as m, importManagers as mt, DEFAULT_FILTERS as n, useofc360 as nn, fetchManagers as nt, DEPARTMENT_GROUPS as o, fmtDate as ot, PROMOTION_STATUS_OPTIONS as p, importDepartments as pt, deleteGoal as q, upsertOffer as qt, DEFAULT_FILTERS$1 as r, validateEmail as rn, fetchOrgChart as rt, DEPARTMENT_VALUES as s, fmtRelative as st, BASE_URL as t, useSendChatMessageMutation as tn, fetchManagerById as tt, MANAGER_FORM_EMPLOYMENT_TYPE_OPTIONS as u, getExpandedGroupsForValue as ut, STATUS_OPTIONS as v, isParentGroupValue as vt, apiInstance as w, optimisticMoveStage as wt, addFeedback as x, mapApiFieldErrors as xt, activateEmployee as y, labelFor as yt, createDepartment as z, updateEmployee as zt };
